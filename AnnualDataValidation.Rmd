...
output:
  html_document:
    df_print: paged
params:
  preppedData: "./regionalPreppedData/gavin.rds"
---
...
**Stations selected include: `r data %>% dplyr::pull(STATION_NAME_FULL) %>% unique`.**
...
### PM~2.5~
```{r pm25sas}
# GOT TO HERE - WORKING NOW BUT DUPLICATE ROWS COMING OUT OF PM25SAS CALCULATION, WITH NA IN STATION_NAME_FULL
...
pm25SASFcn(data %>% dplyr::filter(STATION_NAME_FULL %in% station[3] &
                                    PARAMETER %in% "PM25"))
...
suppressWarnings(
  ...
  # station<-"Abbotsford A Columbia Street"
    ...
    p<-ggplot(data = data %>% 
      ...
      facet_wrap( ~ STATION_NAME_FULL, scales = "free") 
