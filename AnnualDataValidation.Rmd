...
if(nrow(pm25sas != 0)) {
   ...
   DT::datatable(
    pm25sas,
    caption = "AQS calculated statistics. There will be one fewer valid hour compared to ECMS stats, I'll try to resolve this for next year.",
    options = list(
      scrollX = TRUE,
      scroller = TRUE,
      scrollY = 300,
...
df_stat_header <- readxl::read_excel(
...
df_stat_official <- readxl::read_excel(
  file.path(sasReports,
            "2019_PM2.5_UNSUPPRESSED.xlsx"))
...
```{r pm25sasECMSDT}
if(nrow(df_stat_official)!=0) {
 
   DT::datatable(
    df_stat_official %>% dplyr::select(-c(`EMS ID`,
                                          `NAPS ID`,
                                          REGION,
                                          OWNER,
                                          UNIT)),
    caption = "ECMS calculated statistics.",
    options = list(
      scrollX = TRUE,
      scroller = TRUE,
      scrollY = 300,
  ...
  print("There are no ECMS SAS stats to display.")
}
```
