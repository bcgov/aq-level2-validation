...
# data <- readr::read_rds("2022data.rds")
...
# save list of stations in yearToValidate:
allStations<-data %>%
...
# do an anti-join with list of stations from all of data to see which stations need to be removed
# i.e. need to remove stations that don't have data in yearToValidate (these would include
# stations that have been decomissioned in prevYrWind.rds)
  dplyr::anti_join(data %>%
                   dplyr::select(STATION_NAME) %>%
                   dplyr::distinct(.) %>%
                   dplyr::arrange(STATION_NAME),
                   allStations) %>% utils::View(.) # delete these stations from the preppedData folder and re-run
  # 02-compileBooks.R
