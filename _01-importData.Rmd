...
prevYrWind <-
  ...
  purrr::map_dfr((yearToValidate - 6):(yearToValidate - 1),
                 
                 function(windYr) {
                   
                   purrr::map_dfr(c("WSPD_SCLR", "WDIR_VECT"),
  ...
  # filter for last X years of data:
  dplyr::filter(format(DATE_PST, "%Y") %in% (yearToValidate - 6):(yearToValidate - 1))
...
# test <- data %>% 
#   dplyr::filter(STATION_NAME=="Colwood City Hall" & 
#                   PARAMETER == "WSPD_SCLR")
#   
# test_padded<-NAfillFcn(test )%>%
...
#   dplyr::filter(n()>1)
# 
# utils::View(test %>% arrange(DATE_PST))
# 
# 
# test %>% distinct(PARAMETER,INSTRUMENT)
```

Pad time series with NA's to complete the annual time series. Note this includes down to the STATION_NAME_FULL looping level. (updated in Mar 2020). Looping through STATION_NAME isn't sufficient because it can lead to duplicates (between, for example, Birchbank_Golf_Course and Birchbank_Golf_Course_60). 2021 - is this still true?
