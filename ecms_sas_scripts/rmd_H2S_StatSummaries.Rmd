---
title:  "`r params$station` H~2~S Statistical Summary"
...
if (0)
{
  ...
  station <- 'Bessborough 237 Road'
  parameter <- 'h2s'
  sas_summary <- 'A:/Air/Operations ORCS/Data/05_CAAQS/R Summaries/2019_UNSUPPRESSED/Submitted_200228/2019_H2S_UNSUPPRESSED.xlsx'
...
__Table 1.__ Statistical Summary of H~2~S at `r station`. Use vertical arrows next to column name to sort on that category. Use horizontal scrollbar to view more columns. 
...
## Percentiles of H~2~S based on hourly values


```{r 1-Hr Annual Average, echo = FALSE, warning = FALSE, message = FALSE}
...
#annual average values based on hourly  values
  df_stat_official %>%
    ...
    ggtitle(label = paste(station,':H2S',sep='')) +
    ylab('H2S Hourly Concentration, ppb')
  

```

__Figure 1.__ Plot of _Annual Hourly (1-hr) Measurements_ from the H~2~S analyzer at `r station`. Box plot represents 25th and 75th percentile. Whiskers represent the minimum and the 1.5 x IQR (Interquantile Range). Grey diamond represents the maximum (100 percentile). Red dot represents the annual average.

## Percentiles of the Daily (24-hr) H~2~S Values
...
df_stat_official%>%
  ...
  geom_boxplot(aes(x=YEAR,lower=`25%(day)`,middle=`50%(day)`,upper=`75%(day)`,
                   ...
                   ymin=`0%(day)`,
                   group = YEAR),stat='identity',colour = 'blue') +
  #geom_point(aes(x=YEAR,y=`ANNUAL 99P_D1HM`),colour='red',size=1) +
  ...
  theme(axis.text.x =element_text(angle=45),axis.title.x=element_blank()) +
  ggtitle(label = paste(station,':H2S',sep='')) +
  ylab('Daily (24-hr) H2S Values, ppb') 


```

__Figure 2.__ Plot of Daily(24-hr) H~2~S concentration at `r station`. Box plot represents 25th and 75th percentile of the daily TRS measurements. Whiskers represent the minimum and the 1.5 x IQR (Interquantile Range). Grey diamond represents the maximum (100 percentile).
...
__Figure 3.__ Valid data capture of H~2~S measurements at `r station`. Red horizontal line shows the data capture criteria.
