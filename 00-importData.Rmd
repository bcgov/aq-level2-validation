...
(stations <-
   readr::read_csv(file.path("./regional_stations", params$stations)) %>%
...
Testing and fixing for _60 naming convention.


```{r TESTING}
### TESTING
# Quesnel Redwood has data until 2019-08-29 0700, NAFillFcn isn't working
quesnelRedwood<-regionalData %>%
  dplyr::filter(STATION_NAME_FULL %in% toupper("Quesnel Redwood"))

saveRDS(quesnelRedwood,"quesnelRedwood2019.rds")

quesnelRedwood_padded<-NAfillFcn(quesnelRedwood)
  
quesnelRedwood_padded %>%
   summarise(maxDate=max(DATE_PST,na.rm = TRUE),
             minDate=min(DATE_PST,na.rm=TRUE))

tail(quesnelRedwood_padded)

summary(quesnelRedwood_padded)

regionalData_padded$STATION_NAME_FULL %>% unique

saveRDS(regionalData,"gavin2019.rds") 

 # REMOVING _60 STATIONS - NOT DESIRED
data<-feather::read_feather("unverifiedData.feather") %>%
  dplyr::filter((STATION_NAME %in% stations) &
